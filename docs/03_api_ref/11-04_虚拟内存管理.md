...
将虚拟地址 virPtrSrc 映射到虚拟地址 virPtrDst 对应的物理地址，可以通过 linkIdx 选择 HCCS 链路或者 SIO 链路。
...
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| virPtrDst | 输入 | 待映射的虚拟地址。|
| size | 输入 | 虚拟地址所属内存块总大小。 |
| virPtrSrc | 输入 | 被映射的源虚拟地址。 |
| linkIdx | 输入 | 链路标识。 | 0 标识SIO，1 标识HCCS
