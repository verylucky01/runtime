...
在调用本接口之前，请先使用[aclmdlRIGetTasksByStream](#aclmdlRIGetTasksByStream)接口获取指定Stream中的所有任务，然后使用[aclmdlRITaskGetType](#aclmdlRITaskGetType)接口确认当前的任务是Cube Core和Vector Core上的计算任务，再调用本接口查询具体的属性配置。
