...
source_value=$(grep "Server: verify data from shared memory, value" $file_path_server | awk -F'=' '{gsub(/^ +| +$/, "", $2); print $2}' | head -n 1)

if [ -n "$source_value" ]; then
    echo "[SUCCESS] Server completed successfully. Source data: $source_value"
else
