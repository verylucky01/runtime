...
typedef enum {
    RT_KERNEL_ATTR_TYPE_AICORE = 0,
    RT_KERNEL_ATTR_TYPE_CUBE = 1,
    RT_KERNEL_ATTR_TYPE_VECTOR = 2,
    RT_KERNEL_ATTR_TYPE_MIX = 3,
    RT_KERNEL_ATTR_TYPE_AICPU = 100,
} rtKernelAttrType;
...
/**
 * @ingroup profiling_base
 * @brief start adc profiler.
 * @return RT_ERROR_NONE for ok
 * @return RT_ERROR_INVALID_VALUE for error input
 */
RTS_API rtError_t rtStartADCProfiler(void **addr, uint32_t length);

/**
 * @ingroup profiling_base
 * @brief stop adc profiler.
 * @return RT_ERROR_NONE for ok
 * @return RT_ERROR_INVALID_VALUE for error input
 */
RTS_API rtError_t rtStopADCProfiler(void *addr);
 ...
 * @return RT_ERROR_INVALID_VALUE for error input
 */
RTS_API rtError_t rtGetBinBuffer(const rtBinHandle binHandle, const rtBinBufferType_t type, void **bin,
                                 uint32_t *binSize);
 ...
 * @param [in] infoPtr  cache data addr
 * @param [in] infoSize cache data size
 * @return RT_ERROR_NONE for ok
 * @return RT_ERROR_INVALID_VALUE for error input
 */
RTS_API rtError_t rtCacheLastTaskOpInfo(const void * const infoPtr, const size_t infoSize);
