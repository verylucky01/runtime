...
output_progress() {
    new_echo "INFO" "runtime upgrade upgradePercentage:$1%"
    log "INFO" "runtime upgrade upgradePercentage:$1%"
}

##########################################################################
log "INFO" "step into run_runtime_upgrade.sh ......"
...
new_upgrade() {
    if [ ! -d "${sourcedir}" ]; then
        log "INFO" "no need to upgrade runtime files."
        return 0
    fi
    output_progress 10
