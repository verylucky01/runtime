...
namespace cce {
namespace runtime {
static void MacroInitDc(rtSocType_t socType, RtMacroValue &value)
{
    UNUSED(socType);
    value.maxPersistTaskNum = 32760U;
    value.maxTaskNumPerStream = 2042U;
    value.maxSinkTaskNum = 1024U * 2042U;
    value.maxSupportTaskNum = 1024U * 2042U;
...
static DevDynInfoProcFunc CHIP_DC_PROC_FUNC = {
    .macroInitFunc = &MacroInitDc,
};

REGISTER_DEV_INFO_PROC_FUNC(CHIP_DC, CHIP_DC_PROC_FUNC);
}
}