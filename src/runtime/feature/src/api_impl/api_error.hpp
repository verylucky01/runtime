 ...
 */
#ifndef CCE_RUNTIME_API_ERROR_HPP
#define CCE_RUNTIME_API_ERROR_HPP
...
public:
    ...
    ~ApiErrorDecorator() override = default;
    ...
    rtError_t ProfilerStart(const uint64_t profConfig, const int32_t numsDev,
        uint32_t * const deviceList, const uint32_t cacheFlag, const uint64_t profSwitchHi) override;
    rtError_t ProfilerStop(const uint64_t profConfig, const int32_t numsDev, uint32_t * const deviceList, const uint64_t profSwitchHi) override;
    ...
    void SetImpl(Api * const impl)
    {
        ApiDecorator::impl_ = impl;
    }
...
private:
    ...
    rtError_t GetMemcpyConfigAttr(rtMemcpyAttribute_t* attr, RtMemcpyCfgInfo* configInfo) const;
    rtError_t CheckMemcpyCfg(const RtMemcpyCfgInfo* const configInfo, const bool isAsync) const;
    rtError_t CheckStreamFlags(const uint32_t flags) const;
    rtError_t CheckMemcpyAttribute(const rtMemcpyKind kind, const void * const dst, const void * const src) const;
    rtError_t GetThreadLastTaskId(uint32_t * const taskId) override;
    rtError_t MemcpyKindAutoCorrect (const rtMemLocationType srcLocationType, const rtMemLocationType dstLocationType,rtMemcpyKind_t *kind) const;
...
namespace std {
template <>
struct hash<rtMemcpyKind> {
    std::size_t operator()(const rtMemcpyKind k) const noexcept {
        return static_cast<size_t>(k);
    }
};
}  // namespace std
#endif  // CCE_RUNTIME_LOGGER_HPP
