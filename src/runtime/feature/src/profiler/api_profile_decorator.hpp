 ...
 */
#ifndef CCE_RUNTIME_API_PROFILE_DECORATOR_HPP
#define CCE_RUNTIME_API_PROFILE_DECORATOR_HPP
...
class ApiProfileDecorator : public ApiDecorator {
public:
    ApiProfileDecorator(Api * const impl, Profiler * const prof);
    ~ApiProfileDecorator() override
    {
        profiler_ = nullptr;
        return;
    }
    ...
    rtError_t MemAdvise(void *devPtr, const uint64_t count, const uint32_t advise) override;
    ...
    rtError_t MemcpyAsyncPtr(void * const memcpyAddrInfo, const uint64_t destMax,
        const uint64_t count, Stream * const stm, const rtTaskCfgInfo_t * const cfgInfo, const bool isMemcpyDesc = false) override;
...
#endif  // CCE_RUNTIME_API_PROFILE_DECORATOR_HPP
