...
TEST_F(UpdateDevPropertiesValueTest, UpdateDevPropertiesValue)
{
    ...
    EXPECT_EQ(props.maxSupportTaskNum, 67108864U);
    EXPECT_EQ(props.stubEventCount, 65536U);
    EXPECT_EQ(props.maxReportTimeoutCnt, 360);
    EXPECT_EQ(props.rtcqDepth, 0U);
    EXPECT_EQ(props.baseAicpuStreamId, 32768U);
    EXPECT_EQ(props.expandStreamRsvTaskNum, 0U);
    EXPECT_EQ(props.expandStreamSqDepthAdapt, 0U);
    EXPECT_EQ(props.expandStreamAdditionalSqeNum, 1U);
    EXPECT_EQ(props.rsvAicpuStreamNum, 1024U);
    EXPECT_EQ(props.maxPhysicalStreamNum, 2038U);
    EXPECT_EQ(props.maxAllocStreamNum, 32768U);
