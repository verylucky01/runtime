...
static constexpr uint32_t INVALID_TASK_ID = 0xffffU;
    ...
    struct halFaultEventInfo* eventInfo, uint32_t len, uint32_t *eventCount)
{
    ...
    *eventCount = 1;
    eventInfo[0].event_id = 0x80CD8008;
    return DRV_ERROR_NONE;
}
        ...
        if (moduleType == MODULE_TYPE_SYSTEM && infoType == INFO_TYPE_VERSION) {
            ...
            *value = g_device_driver_version_stub;
            printf("\r\n halGetDeviceInfo:: moduleType = %d, infoType = %d, g_device_driver_version_stub = %d",
                moduleType,
                infoType,
                g_device_driver_version_stub);
        } else if (moduleType == MODULE_TYPE_AICORE && infoType == INFO_TYPE_CORE_NUM) {
            *value = g_device_driver_aicore_stub;
            printf("\r\n halGetDeviceInfo:: moduleType = %d, infoType = %d, g_device_driver_version_stub = %d",
                moduleType,
                infoType,
                g_device_driver_aicore_stub);
        } else if (moduleType == MODULE_TYPE_VECTOR_CORE && infoType == INFO_TYPE_CORE_NUM) {
            *value = g_device_driver_vector_core_stub;
            printf("\r\n halGetDeviceInfo:: moduleType = %d, infoType = %d, g_device_driver_version_stub = %d",
                moduleType,
                infoType,
                g_device_driver_vector_core_stub);
        } else {
            *value = 0;
        }
    }
...
drvError_t halSqCqAllocate(uint32_t devId, struct halSqCqInputInfo *in, struct halSqCqOutputInfo *out)
{
    ...
    switch (type) {
        case DRV_NORMAL_TYPE: {
            ...
            g_sqCqInfo[devId][out->sqId].streamId = streamId;
            g_sqCqInfo[devId][out->sqId].cqId = out->cqId;
            g_sqCqInfo[devId][out->sqId].sqId = out->sqId;
            g_sqCqInfo[devId][out->sqId].type = type;
            g_sqCqInfo[devId][out->sqId].taskId = 0xffff;
            g_sqCqInfo[devId][out->sqId].valid = true;
            break;
        }
        case DRV_LOGIC_TYPE: {
            {
...
drvError_t halSqCqFree(uint32_t devId, struct halSqCqFreeInfo *in)
{
    ...
    switch (type) {
        case DRV_NORMAL_TYPE: {
            ...
            g_sqCqInfo[devId][sqId].sqTail = 0;
            g_sqCqInfo[devId][sqId].taskId = 0xffff;
            g_sqCqInfo[devId][sqId].cqId = 0xffffffff;
            g_sqCqInfo[devId][sqId].sqId = 0xffffffff;
            break;
        }
        case DRV_LOGIC_TYPE: {
            {
...
drvError_t StubHalSqTaskSendNormalType(uint32_t devId, struct halTaskSendInfo *info)
{
    ...
    uint32_t sqe_num = info->sqe_num;
    std::lock_guard<std::mutex> lock(g_sqCqInfoMutex);
    // 1: submit a new task. update tail
    uint32_t tail = g_sqCqInfo[devId][sqId].sqTail + sqe_num;
    g_sqCqInfo[devId][sqId].sqTail = (tail % MAX_SQCQ_NUM);
    // 2: Simulate stars,  finish the task now. update head
    uint32_t head = g_sqCqInfo[devId][sqId].sqHead + sqe_num;
    g_sqCqInfo[devId][sqId].sqHead = (head % MAX_SQCQ_NUM);
    rtStarsSqe_t *sqe = (rtStarsSqe_t*)(info->sqe_addr);
    cce::runtime::rtStarsSqeHeader_t* sqeHead = &(sqe->commonSqe.sqeHeader);
    uint32_t needCq = sqeHead->wr_cqe;
    if (needCq == 0) {
        RT_LOG(RT_LOG_INFO, "devId=%d, sqId=%d, head=%d, tail=%d, no cqe.",
         devId, sqId, g_sqCqInfo[devId][sqId].sqHead, g_sqCqInfo[devId][sqId].sqTail);
        return DRV_ERROR_NONE;
    }
    // 3: Simulate stars, Write cqe now
    uint32_t cqId = g_sqCqInfo[devId][sqId].cqId;

    uint32_t streamId = sqeHead->rt_stream_id;
    streamId &= 0x7FFFU;
    uint32_t taskId = sqeHead->task_id;
    ...
    reportInfo.sqHead = g_sqCqInfo[devId][sqId].sqHead;
    reportInfo.errorCode = 0;
    reportInfo.errorType = 0;
    g_logicCqReport[devId][cqId].push(reportInfo);
...
drvError_t halCqReportRecv(uint32_t devId, struct halReportRecvInfo *info)
{
    ...
    uint32_t streamId = info->stream_id;
    uint32_t taskId = info->task_id;
    if (streamId == MAX_UINT32_NUM) {
        // dvppgrp todo
        RT_LOG(RT_LOG_ERROR, "Unsupported, please add");
        return DRV_ERROR_NONE;
    }
    ...
    for (auto &a: g_sqCqInfo[devId]) {
        ...
        uint16_t softRecord = (uint16_t)a.streamId;
        if (softRecord == streamId) {
            sqId = a.sqId;
            break;
        }
    }
    std::lock_guard<std::mutex> lock(g_logicCqReportMutex);
    if (g_logicCqReport[devId][sqId].empty()) {
...
drvError_t halSqCqQuery(uint32_t devId, struct halSqCqQueryInfo *info)
{
    ...
    drvSqCqType_t type = info->type;
    drvSqCqPropType_t prop = info->prop;
    switch (type) {
        case DRV_NORMAL_TYPE: {
            std::lock_guard<std::mutex> lock(g_sqCqInfoMutex);
            if (!g_sqCqInfo[devId][sqId].valid) {
                RT_LOG(RT_LOG_ERROR, "sqId:%u has been released", sqId);
                return DRV_ERROR_INVALID_VALUE;
            }
            if (prop == DRV_SQCQ_PROP_SQ_HEAD) {
                info->value[0] = g_sqCqInfo[devId][sqId].sqHead;
                RT_LOG(RT_LOG_INFO, "devId=%d, sqId=%d, head=%d.", devId, sqId, g_sqCqInfo[devId][sqId].sqHead);
            } else if (prop == DRV_SQCQ_PROP_SQ_TAIL) {
                info->value[0] = g_sqCqInfo[devId][sqId].sqTail;
                RT_LOG(RT_LOG_INFO, "devId=%d, sqId=%d, tail=%d.", devId, sqId, g_sqCqInfo[devId][sqId].sqTail);
            } else if (prop == DRV_SQCQ_PROP_SQ_REG_BASE) {
                info->value[0] = 1;
            }
            break;
        }
        default: {
            RT_LOG(RT_LOG_ERROR, "type:%d, use default value", static_cast<int32_t>(type));
            if (prop != DRV_SQCQ_PROP_SQ_CQE_STATUS) {
                info->value[0] = 1;
            } else {
                info->value[0] = 0;
            }
...
{
    if (share_handle == nullptr || server_id == nullptr) {
        return DRV_ERROR_INVALID_VALUE;
    }
    return DRV_ERROR_NONE;
}
...
drvError_t halStreamTaskFill(uint32_t dev_id, uint32_t stream_id, void* stream_mem,
