...
drvError_t halGetDeviceInfo(uint32_t devId, int32_t moduleType, int32_t infoType, int64_t *value)
{
    ...
    std::vector<Info> info = {
        {MODULE_TYPE_SYSTEM, INFO_TYPE_VERSION, static_cast<int64_t>(PLATFORMCONFIG_CLOUD_V1)},
        {MODULE_TYPE_SYSTEM, INFO_TYPE_CORE_NUM, g_device_driver_version_stub},
        {MODULE_TYPE_AICORE, INFO_TYPE_CORE_NUM_LEVEL,
         static_cast<int64_t>(cce::runtime::Runtime::AICORE_NUM_LEVEL_PG)},
    };

    for (auto &a : info) {
        if ((moduleType == a.module) && (infoType == a.type)) {
            *value = a.value;
             printf("\r\n halGetDeviceInfo:: moduleType = %d, infoType = %d, version value = %d",
                moduleType, infoType, *value);
            break;
        }
    }
    return DRV_ERROR_NONE;
