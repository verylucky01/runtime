...
TEST_F(UpdateDevPropertiesValueTest, UpdateDevPropertiesValue)
{
    ...
    DevProperties props;
    EXPECT_EQ(GET_DEV_PROPERTIES(CHIP_MINI_V3, props), RT_ERROR_NONE);
    EXPECT_EQ(props.maxPersistTaskNum, 65535U);
    ...
    EXPECT_EQ(props.maxPhysicalStreamNum, 480U);
    EXPECT_EQ(props.maxAllocStreamNum, 480U);
    EXPECT_EQ(props.rtsqDepth, 2048U);
    EXPECT_EQ(props.maxTaskNumPerStream, 2010U);
