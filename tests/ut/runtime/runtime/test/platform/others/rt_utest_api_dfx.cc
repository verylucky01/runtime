...
TEST_F(ApiCloudDisableThreadDfxTest, streamCreateFail3)
{
    rtStream_t stm;
...
TEST_F(ApiCloudDisableThreadDfxTest, streamCreateFail4)
{
    rtStream_t stm;
    MOCKER(halSqMsgSend).stubs().will(returnValue(DRV_ERROR_IOCRL_FAIL));

    rtError_t error1 = rtEventRecord(event_, stream_);
    EXPECT_EQ(error1, ACL_ERROR_RT_DRV_INTERNAL_ERROR);
    GlobalMockObject::verify();
}